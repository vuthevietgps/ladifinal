...
document.getElementById('checkForm').addEventListener('submit', function(e) {
  ...
  // Prepare message
  const message = `Tôi muốn kiểm tra phù hiệu xe:
  
Họ tên: ${fullname}
SĐT: ${phone}
Loại xe: ${vehicleType}
Trọng tải: ${weight} tấn
Mục đích: ${purpose}

Tôi cần được tư vấn và hỗ trợ làm phù hiệu xe.`;
...
document.querySelectorAll('[id^="cta"]').forEach(btn => {
  btn.addEventListener('click', function() {
    const message = 'Tôi muốn kiểm tra phù hiệu xe miễn phí. Hãy hỗ trợ tôi.';
    const zaloLink = `https://zalo.me/0363614511?text=${encodeURIComponent(message)}`;
    window.open(zaloLink, '_blank');
  });
});
...
document.getElementById('checkForm')?.addEventListener('submit', () => {
  trackEvent('Form_Submit', { type: 'consultation_check' });
});
