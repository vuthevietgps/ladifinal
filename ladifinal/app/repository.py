...
def update_landing(landing_id: int, data: Dict[str, Any]):
    ...
    cur = db.execute(sql, [data[c] for c in cols] + [landing_id])
    db.commit()
...
def get_homepage() -> Optional[Dict[str, Any]]:
    db = get_db()
    row = db.execute("SELECT * FROM landing_pages WHERE page_type='homepage' ORDER BY updated_at DESC LIMIT 1").fetchone()
    return row_to_dict(row) if row else None
...
def delete_landing(landing_id: int):
    db = get_db()
    cur = db.execute("DELETE FROM landing_pages WHERE id=?", (landing_id,))
    db.commit()
    return cur.rowcount > 0
